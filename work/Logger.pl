...
atomic_list_concat(var_List,var_Atom):-atomic_list_concat(var_List,' ',var_Atom).

atomic_list_concat([],var__,'').

atomic_list_concat([var_H|var_T],var_Sep,var_Atom):-term_to_atom_safe(var_H,var_HAtom),atomic_list_concat(var_T,var_Sep,var_Rest),(var_Rest==''->var_Atom=var_HAtom;atom_concat(var_HAtom,var_Sep,var_Temp),atom_concat(var_Temp,var_Rest,var_Atom)).

term_to_atom_safe(var_Term,var_Atom):-write_term_to_codes(var_Term,var_Codes,[quoted(false)]),atom_codes(var_Atom,var_Codes).

underscore_to_space(var_AtomIn,var_AtomOut):-atom_chars(var_AtomIn,var_CharsIn),replace_underscore(var_CharsIn,var_CharsOut),atom_chars(var_AtomOut,var_CharsOut).

replace_underscore([],[]).

replace_underscore(['_'|var_T],[' '|var_R]):-replace_underscore(var_T,var_R).

replace_underscore([var_H|var_T],[var_H|var_R]):-var_H\='_',replace_underscore(var_T,var_R).

eve(def_urgency(var_TP,var_WR,var_PT)):-atom_concat('===== New Emergency for the patient ',var_PT,var_Temp),atom_concat(var_Temp,' in the Ward ',var_Temp2),atom_concat(var_Temp2,var_WR,var_Temp3),atom_concat(var_Temp3,' : ',var_Temp4),atom_concat(var_Temp4,var_TP,var_Temp5),atom_concat(var_Temp5,' =====',var_Text),atom_concat('Ward_',var_WR,var_Ward),logger(var_Text,var_Ward).

eve(taking_charge_emergency(var_Patient,var_Ward)):-write('taking_charge_emergency'),nl,atom_concat('Emergency_for_patient_',var_Patient,var_Temp),atom_concat(var_Temp,'_taken_in_charge',var_Temp2),underscore_to_space(var_Temp2,var_Text),logger(var_Text,var_Ward).

eve(emergency_handled(var_Patient,var_Ward)):-atom_concat('[END EMERGENCY]: the_emergency_of_',var_Patient,var_Temp),atom_concat(var_Temp,'_has_been_correctly_handled',var_Temp2),underscore_to_space(var_Temp2,var_Text),logger(var_Text,var_Ward).

eve(porcodio):-write('porcodio').

eve(human_resource_request(var_N,var_D,var_R,var_Rec_Ward,var_Patient)):-write('human_resource_request'),nl,var_Req_map=['Nurses'-var_N,'Doctors'-var_D,'Resuscitators'-var_R],atomic_list_concat(var_Req_map,var_Req),atom_concat('HR_request_from_',var_Rec_Ward,var_Temp),atom_concat(var_Temp,'_of_',var_Temp2),atom_concat(var_Temp2,var_Req,var_Temp3),underscore_to_space(var_Temp3,var_Text),logger(var_Text,'HRCoordinator_instance').

eve(human_resource_lending(var_N,var_D,var_R,var_Receiver_Ward,var_Sender_Ward,var_Patient)):-write('human_resource_lending'),nl,var_Req_map=['Nurses'-var_N,'Doctors'-var_D,'Resuscitators'-var_R],atomic_list_concat(var_Req_map,var_Req),atom_concat('HR_lended_from_',var_Sender_Ward,var_Temp),atom_concat(var_Temp,'_to_',var_Temp2),atom_concat(var_Temp2,var_Receiver_Ward,var_Temp3),atom_concat(var_Temp3,':_',var_Temp4),atom_concat(var_Temp4,var_Req,var_Temp5),underscore_to_space(var_Temp5,var_Text),logger(var_Text,'HRCoordinator_instance').

eve(human_resource_restitution(var_N,var_D,var_R,var_Patient,var_Receiver_Ward,var_Sender_Ward)):-write('human_resource_restitution'),nl,var_Req_map=['Nurses'-var_N,'Doctors'-var_D,'Resuscitators'-var_R],atomic_list_concat(var_Req_map,var_Req),atom_concat('HR_restitution_from_',var_Receiver_Ward,var_Temp),atom_concat(var_Temp,'_to_',var_Temp2),atom_concat(var_Temp2,var_Sender_Ward,var_Temp3),atom_concat(var_Temp3,':_',var_Temp4),atom_concat(var_Temp4,var_Req,var_Temp5),underscore_to_space(var_Temp5,var_Text),logger(var_Text,'HRCoordinator_instance').

eve(human_resource_restore_ward(var_Old_N,var_Old_D,var_Old_R,var_New_N,var_New_D,var_New_R,var_Ward)):-write('human_resource_restore_ward'),nl,var_Old_equipe=['Nurses'-var_Old_N,'Doctors'-var_Old_D,'Resuscitators'-var_Old_R],var_New_equipe=['Nurses'-var_New_N,'Doctors'-var_New_D,'Resuscitators'-var_New_R],atomic_list_concat(var_Old_equipe,var_Old_equipe_atom),atomic_list_concat(var_New_equipe,var_New_equipe_atom),atom_concat('Restoring_lended_HR_:_',var_Old_equipe_atom,var_Temp),atom_concat(var_Temp,'_current_equipe_',var_Temp2),atom_concat(var_Temp2,var_New_equipe_atom,var_Temp3),underscore_to_space(var_Temp3,var_Text),logger(var_Text,var_Ward).

eve(assign_rrt(var_Result,var_Patient,var_From)):-(var_Result==0->atom_concat('Not_enough_HR_in_the_Ward_for_the_emergency_of_the_patient_',var_Patient,var_Temp),underscore_to_space(var_Temp,var_Text);atom_concat('RRT_regularly_assigned_to_the_patient_',var_Patient,var_Temp),underscore_to_space(var_Temp,var_Text)),logger(var_Text,var_From).

logger(var_Text,var_Sender):-log_file(var_File),open(var_File,append,var_Stream),def_timestamp(var_TimeStamp),format(var_Stream,'[~w] [~w] ~w~n',[var_TimeStamp,var_Sender,var_Text]),close(var_Stream).

format_number(var_N,var_Formatted):-number_chars(var_N,var_Chars),(var_N<10->atom_chars(var_Formatted,['0'|var_Chars]);atom_chars(var_Formatted,var_Chars)).

def_timestamp(var_TimeStamp):-datime(datime(var_Y,var_M,var_D,var_H,var_Min,var_S)),format_number(var_Y,var_YF),format_number(var_M,var_MF),format_number(var_D,var_DF),format_number(var_H,var_HF),format_number(var_Min,var_MinF),format_number(var_S,var_SF),atom_concat(var_YF,'-',var_T1),atom_concat(var_T1,var_MF,var_T2),atom_concat(var_T2,'-',var_T3),atom_concat(var_T3,var_DF,var_T4),atom_concat(var_T4,' ',var_T5),atom_concat(var_T5,var_HF,var_T6),atom_concat(var_T6,':',var_T7),atom_concat(var_T7,var_MinF,var_T8),atom_concat(var_T8,':',var_T9),atom_concat(var_T9,var_SF,var_TimeStamp).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_460627,var_Ontology,_460631),_460621),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_460665)),a(message(var_Ag,propose(var_A,[_460665],var_AgI))),retractall(ext_agent(var_Ag,_460703,var_Ontology,_460707)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_460501,var_Ontology,_460505),_460495),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_460571,var_Ontology,_460575)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_460389,var_Ontology,_460393),_460383),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_460445,var_Ontology,_460449)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_459953),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_459987),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_459769).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_459617),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_459565),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_459441,_459443)),clause(agente(_459463,_459465,_459467,var_S),_459459),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_459235,_459237,_459239,var_S),_459231),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_459335,[]),repeat,read(_459335,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_459335).
