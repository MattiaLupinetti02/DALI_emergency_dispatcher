...
critical_hr_out(var_Patient,var_HR):-patient_taken_in_charge(var_B),var_B==0,trashold_hr(var_V),var_HR>=var_V,write(var_Patient),write(' has a high hr: '),write(var_HR),nl,ward(var_WardName),config(var_A),var_A==1,p_name(var_Pname),atom_concat('Ward_',var_WardName,var_InstanceWardName),my_instance_name(var_InstanceName),write('from: '),write(var_InstanceName),nl,write('to: '),write(var_InstanceWardName),nl,a(message(var_InstanceWardName,send_message(alarm('high_hr',var_WardName,var_Pname),var_InstanceName))).

critical_oo_out(var_Patient,var_OO):-patient_taken_in_charge(var_B),var_B==0,trashold_o(var_V),var_V>var_OO,write(var_Patient),write(' has a low SpO2: '),write(var_OO),nl,ward(var_WardName),config(var_A),var_A==1,p_name(var_Pname),atom_concat('Ward_',var_WardName,var_InstanceWardName),my_instance_name(var_InstanceName),write('from: '),write(var_InstanceName),nl,write('to: '),write(var_InstanceWardName),nl,a(message(var_InstanceWardName,send_message(alarm('low_o2',var_WardName,var_Pname),var_InstanceName))).

eve(taking_charge_emergency):-write('Stop sending: emergy handled'),nl,retractall(patient_taken_in_charge(var__)),assertz(patient_taken_in_charge(1)).

eve(emergency_handled):-ward(var_Ward_name),format('Emergency handled by ward ~w~n',[var_Ward_name]),retractall(patient_taken_in_charge(var__)),assertz(patient_taken_in_charge(0)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_416751,var_Ontology,_416755),_416745),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_416789)),a(message(var_Ag,propose(var_A,[_416789],var_AgI))),retractall(ext_agent(var_Ag,_416827,var_Ontology,_416831)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_416625,var_Ontology,_416629),_416619),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_416695,var_Ontology,_416699)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_416513,var_Ontology,_416517),_416507),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_416569,var_Ontology,_416573)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_416077),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_416111),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_415893).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_415741),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_415689),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_415565,_415567)),clause(agente(_415587,_415589,_415591,var_S),_415583),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_415359,_415361,_415363,var_S),_415355),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_415459,[]),repeat,read(_415459,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_415459).
