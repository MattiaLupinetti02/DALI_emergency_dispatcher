...
SICSTUS_HOME=/project/prog_env
MAIN_HOME=/project/prog_env/DALI_emergency_dispatcher
DALI_HOME=/project/prog_env/DALI/src
CONF_DIR=conf
...
for agent_filename in $BUILD_HOME/*; do
    ...
    tmux new-window -t DALI_session -n $agent_base "$current_dir/conf/startagent.sh $agent_base $PROLOG $DALI_HOME"
...
tmux new-window -t DALI_session -n user "$PROLOG --noinfo -l $DALI_HOME/active_user_wi.pl --goal utente."
